...
const i18n = {
  zh: {
    ...
    teachSlashBody: "最上面是常驻台——总控、贸易、物项与许可、第三方尽调；选中一张就一直在上面，开新对话也还在。往下是每张台的窄化入口，同一条线、问题已经指向其中一段。Skill 挂在当前台下面，把一段流程追加给模型。共 {n} 个 Gem、{s} 个 Skill（含 1 个内置）。",
  ...
  en: {
    ...
    teachSlashBody: "Desks at the top \u2014 the coordinator, Trade, Item & licence, Third-party diligence. Pick one and you stay there, new conversations included. Below each are its narrower entries: the same lane with the question already pointed at one part of it. Skills hang under the desk you are at. {n} gems and {s} skills, including the built-in one.",
...
// Folded or not, and how many are in there. The count is on the header because
// that is all a folded drawer shows, and "历史记录" alone says nothing about
// whether opening it is worth the click.
const HISTORY_KEY = "compliance-history-folded";
...
function applyLocale(locale) {
  ...
  renderGemNav();
  renderCaseNav();
  setHistoryFolded(state.historyFolded);
  renderActiveGem();
  updateModePill();
  document.querySelector('#settingsBtn .side-label').textContent = t(state.serverModelConfigured ? "accessSettings" : "modelSettings");
  loadCoverage();
...
$("historyToggle").addEventListener("click", () => setHistoryFolded(!state.historyFolded));
