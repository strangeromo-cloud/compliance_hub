...
更新兜底快照：先在能访问的机器上同步，再把该来源导出成 JSON（`node -e` 读 `readNormalized('china-dual-use')` 即可）写进 `data/fallback/`，补上 `provenance` / `bundledAt` / `note` 字段。兜底文件本身仍然是随仓库提交的普通文件，不进数据库——它属于镜像，不属于运行时数据。
