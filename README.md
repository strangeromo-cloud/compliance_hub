...
设置面板里「模型由服务器提供」后面显示的是 `主机 · 模型名`，两个值都直接来自那台机器的环境变量（`OPENAI_BASE_URL` 的主机部分和 `OPENAI_MODEL`），不带 key、不带路径。

**这两个值必须配套。** base URL 指向 DeepSeek 而模型名还留着 OpenAI 的，每次调用都会失败，而面板照样显示"模型由服务器提供"外加一个看起来没问题的名字——把主机也显示出来，就是为了让这种错配一眼能看出来。
...
Gem 和 Skill 都在输入框敲 `/` 调出来，但**行为不同**：

| | 调用方式 | 进模型的方式 |
|---|---|---|
| Gem | 选中后命令从输入框消失，变成「当前 Gem」这个模式 | 指令和绑定来源拼进问题 |
| Skill | 命令留在文本里 | 服务端从问题里解析掉，SOP 追加到系统提示末尾 |

**但它们共用一个命名空间**：面板同时列出两者，输入框解析一个 `/token` 时并不知道它属于哪一类。所以 `commandOwner()` 一处检查三边——内置八个、自建 Gem、自建 Skill——建任何一个撞名的都返回 409，并说清被谁占了。

它直接读两张表而不是分别 import 那两个模块：让它们互相 import 是一个循环，仅仅因为调用都发生在函数体内才没炸。
